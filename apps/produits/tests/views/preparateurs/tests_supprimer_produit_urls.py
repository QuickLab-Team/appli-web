...
class PreparateursSupprimerProduitTestViews(TestCase):
    ...
    def test_url_supprimer_produit(self):
        response = self.client.get(reverse('produits:supprimer_produit', kwargs={'produit_id': self.produit.id}))
        ...
        response = self.client.post(reverse('produits:supprimer_produit', kwargs={'produit_id': self.produit.id + 1}))
